...
use parser::{ParseError, lexer::Lexer, lexer::TokenValue};

fn iterate_lexer_until_eof<'a>(input: &'a str) -> Result<(), ParseError<'a>> {
    let mut lexer = parser::common::Parser::new(Lexer::new(input)).unwrap();
    while lexer.consume()? != TokenValue::EOF {}
    Ok(())
}
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("insert", |b| {
        b.iter(|| black_box(iterate_lexer_until_eof(query).unwrap()))
    });
