use crate::{
    ...
    sql::parser::{common::ParseError, query::parse_query, tokenizer::tokenize},
};
...
impl Query {
    ...
    pub fn from_individual(line: &'_ str) -> Result<Query, ParseError<'_>> {
...
impl QueryRequest {
    pub fn from_query_request(query: &'_ str) -> Result<Self, ParseError<'_>> {
        let queries = query
            .split(';')
            .filter(|x| !x.is_empty())
            .map(Query::from_individual)
