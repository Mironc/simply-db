...
#[cfg(test)]
mod test {
    use crate::{self as simply_db, hashmap, parse_expr, scalar, scalar_type};
    use crate::{
    ...
    fn unknown_table() {
        ...
        let update = UpdateQuery::new(
            ...
            vec![("age".to_string(), parse_expr!("10").unwrap())],
            None,
        );
    ...
    fn missing_field() {
        ...
        let update = UpdateQuery::new(
            "table".to_string(),
            vec![("name".to_string(), parse_expr!("Alice").unwrap())],
            None,
        );
        assert!(update.execute(&mut db).is_err());
    }
    ...
    fn null_value_on_non_nullable_field() {
        ...
        let update = UpdateQuery::new(
            "table".to_string(),
            vec![("age".to_string(), parse_expr!("NULL").unwrap())],
            None,
        );
        assert!(update.execute(&mut db).is_err());
    }
    ...
    fn type_mismatch() {
        ...
        let update = UpdateQuery::new(
            "table".to_string(),
            vec![("age".to_string(), parse_expr!("'Alice'").unwrap())],
            None,
        );
        assert!(update.execute(&mut db).is_err());
    }
    ...
    fn success() {
        ...
        let update = UpdateQuery::new(
            "table".to_string(),
            vec![("age".to_string(), parse_expr!("30").unwrap())],
            None,
        );
