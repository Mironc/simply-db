...
> *(Note: Experimental `struct` support exists in some areas but is not fully integrated yet).*
...
* **`POST /v1/query`** — Accepts a JSON payload with an SQL query string. 
  * *Request example:* `{"sql": "SELECT * FROM users"}`
  * *Response:* A JSON array containing query execution results (supports multiple results depending on the sent query).
...
> *Note: Data mutations will reflect in the UI only after clicking the `upd` (update) button.*
